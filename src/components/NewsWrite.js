...
const NewsWrite = () => {
  ...
  useEffect(() => {
    ...
    if (postId) {
      axios.get(`http://localhost:5001/api/we/getnews/${postId}`)
        .then(response => {
  ...
  const onWrite = async (e) => {
    ...
    try {
      if (postId) {
        await axios.put(`http://localhost:5001/api/we/updatenews/${postId}`, postData);
        console.log('게시물이 수정되었습니다.');
      } else {
        await axios.post('http://localhost:5001/api/we/writenews', postData);
